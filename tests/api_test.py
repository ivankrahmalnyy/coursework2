...
class TestAPI:
    ...
    def test_app_all_type_count_content(self):
        """Проверяем правильные данные ли получены"""
        response = app.test_client().get('/api/posts', follow_redirect=True)
        assert type(response.json) == list, "Получен не список"
        assert len(response.json) == 8, "полученно не верное количество элементов в списке"
    ...
    def test_app_all_posts_type_check_keys(self):
        ...
        assert keys == first_keys, "Полученные ключи не верны"
